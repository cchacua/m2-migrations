eqpat.df<-read.csv("", header=FALSE)
colnames(eqpat.df)<-c("pat", "PERSON_ID", "INVT_SEQ_NR", "PERSON_CTRY_CODE", "PERSON_NAME","HAN_NAME", "PSN_NAME", "DOC_STD_NAME")
eq08.df<-read.csv("", header=FALSE)
colnames(eq08.df)<-c("pat", "ID", "name")

patnumber<-unique(eqpat.df$pat)
log<-lapply(patnumber,function(x){tryCatch(mergeinvpat(x),error=function(e) print("Error") )})
